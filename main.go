...
func main() {
	...
	if conf.Settings.AppID == 12345 {
		...
		//读取intent
		// webhook_only 为 true 时进入纯 Webhook 模式：跳过 QQ Gateway WebSocket 会话管理器，
		// 完全依赖 QQ 通过 HTTP POST 推送事件。text_intent 中的处理器仍会注册，
		// 确保 Webhook 收到的事件能被正常分发和处理。
		webhookOnlyMode := conf.Settings.WebhookOnly
		if webhookOnlyMode {
			log.Println("[Gensokyo] webhook_only 为 true，进入纯 Webhook 模式，跳过 QQ Gateway WebSocket 连接。")
		}
		...
		if !conf.Settings.SandBoxMode {
			...
			api = botgo.NewOpenAPI(token).WithTimeout(10 * time.Second) // 增加到10秒，支持包含图片的markdown消息
			...
			apiV2 = botgo.NewOpenAPI(token).WithTimeout(10 * time.Second) // 增加到10秒，支持包含图片的markdown消息
			...
			api = botgo.NewSandboxOpenAPI(token).WithTimeout(10 * time.Second) // 增加到10秒，支持包含图片的markdown消息
			...
			apiV2 = botgo.NewSandboxOpenAPI(token).WithTimeout(10 * time.Second) // 增加到10秒，支持包含图片的markdown消息
			log.Println("创建 沙箱 apiv2 成功")
		}

		fix11300 := config.GetFix11300()
		var me *dto.User
		if !fix11300 { // 执行API请求 显示机器人信息
		...
		if !nologin {
			...
			if !fix11300 { //初始化handlers
				handlers.BotID = me.ID
			} else { //初始化handlers
				handlers.BotID = "1234" // 硬编码值
			}
			...
			// 无论是否为 Webhook 模式，都必须注册事件处理器。
			// WebSocket 模式：处理器接收 QQ Gateway WS 推送的事件。
			// Webhook 模式：处理器接收 QQ HTTP POST 事件（server/webhook.go 中 event.ParseAndHandle 使用同一套注册表）。
			// 若此处跳过注册，Webhook 收到的事件将被静默丢弃，导致消息无法收发。
			...
			// 纯 Webhook 模式（webhook_only: true）下跳过 QQ Gateway WebSocket 会话管理器。
			// 事件处理器已在上方注册完毕，Webhook 推来的事件可被正常处理。
			if !webhookOnlyMode {
	...
	r.POST("/"+conf.Settings.WebhookPath, server.CreateHandleValidationSafe(webhookHandler, apiV2))
...
func ATMessageEventHandler() event.ATMessageEventHandler {
	return func(event *dto.WSPayload, data *dto.WSATMessageData) error {
		...
		go p.ProcessGuildATMessage(data, event.RawMessage)
		return nil
	}
}
...
func DirectMessageHandler() event.DirectMessageEventHandler {
	return func(event *dto.WSPayload, data *dto.WSDirectMessageData) error {
		botstats.RecordMessageReceived()
		go p.ProcessChannelDirectMessage(data, event.RawMessage)
		return nil
	}
}
...
func CreateMessageHandler() event.MessageEventHandler {
	return func(event *dto.WSPayload, data *dto.WSMessageData) error {
		botstats.RecordMessageReceived()
		go p.ProcessGuildNormalMessage(data, event.RawMessage)
		return nil
	}
}
...
func InteractionHandler() event.InteractionEventHandler {
	return func(event *dto.WSPayload, data *dto.WSInteractionData) error {
		...
		go p.ProcessInlineSearch(data, event.RawMessage)
		return nil
	}
}
...
func ThreadEventHandler() event.ThreadEventHandler {
	return func(event *dto.WSPayload, data *dto.WSThreadData) error {
		...
		go p.ProcessThreadMessage(data, event.RawMessage)
		return nil
	}
}
...
func GroupATMessageEventHandler() event.GroupATMessageEventHandler {
	return func(event *dto.WSPayload, data *dto.WSGroupATMessageData) error {
		go p.ProcessGroupMessage(data, event.RawMessage)
...
func C2CMessageEventHandler() event.C2CMessageEventHandler {
	return func(event *dto.WSPayload, data *dto.WSC2CMessageData) error {
		go p.ProcessC2CMessage(data, event.RawMessage)
...
func GroupAddRobotEventHandler() event.GroupAddRobotEventHandler {
	return func(event *dto.WSPayload, data *dto.GroupAddBotEvent) error {
		go p.ProcessGroupAddBot(data, event.RawMessage)
		return nil
	}
}
...
func GroupDelRobotEventHandler() event.GroupDelRobotEventHandler {
	return func(event *dto.WSPayload, data *dto.GroupAddBotEvent) error {
		go p.ProcessGroupDelBot(data, event.RawMessage)
		return nil
	}
}
...
func GroupMsgRejectHandler() event.GroupMsgRejectHandler {
	return func(event *dto.WSPayload, data *dto.GroupMsgRejectEvent) error {
		go p.ProcessGroupMsgReject(data, event.RawMessage)
		return nil
	}
}
...
func GroupMsgReceiveHandler() event.GroupMsgReceiveHandler {
	return func(event *dto.WSPayload, data *dto.GroupMsgReceiveEvent) error {
		go p.ProcessGroupMsgRecive(data, event.RawMessage)
		return nil
	}
}
