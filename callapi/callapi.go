...
type ParamsContent struct {
	...
	FileType interface{} `json:"file_type,omitempty"` // 1图片 2视频 3语音 4文件
}
...
func (p *ParamsContent) UnmarshalJSON(data []byte) error {
	...
	case nil: // 当EventID不存在时
		p.EventID = ""
	case float64: // JSON的数字默认被解码为float64
		p.EventID = fmt.Sprintf("%.0f", v) // 将其转换为字符串，忽略小数点后的部分
	case string:
		p.EventID = v
	default:
		return fmt.Errorf("EventID has unsupported type")
	}

	// 处理InteractionID
	switch v := aux.InteractionID.(type) {
	case nil: // 当InteractionID不存在时
		p.InteractionID = ""
	case float64: // JSON的数字默认被解码为float64
		p.InteractionID = fmt.Sprintf("%.0f", v) // 将其转换为字符串，忽略小数点后的部分
	case string:
		p.InteractionID = v
	default:
		return fmt.Errorf("InteractionID has unsupported type")
	}

	// 处理InteractionCode
	switch v := aux.InteractionCode.(type) {
	case nil: // 当InteractionCode不存在时
		p.InteractionCode = 0
	case float64: // JSON的数字默认被解码为float64
		p.InteractionCode = int(v) // 将其转换为整数
	case int:
		p.InteractionCode = v
	case string:
		// 如果是字符串，尝试转换为整数
		if intVal, err := strconv.Atoi(v); err == nil {
			p.InteractionCode = intVal
		} else {
			return fmt.Errorf("InteractionCode string cannot be converted to int: %s", v)
		}
	default:
		return fmt.Errorf("InteractionCode has unsupported type")
	}

	// 处理FileType
	switch v := aux.FileType.(type) {
	case nil:
		p.FileType = 0
	case float64:
		p.FileType = int(v)
	...
	case string:
		if intVal, err := strconv.Atoi(v); err == nil {
			...
			return fmt.Errorf("FileType string cannot be converted to int: %s", v)
		}
	default:
