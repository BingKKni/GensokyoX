...
func auditImage(client *green.Client, objectKey string) (bool, error) {
	...
	// 仅在审核未开通(596) 或审核未通过 时输出完整结果，避免每张通过的图片都被打印
	for _, data := range result.Data {
		for _, r := range data.Results {
			if r.Suggestion != "pass" {
				mylog.Printf("阿里云审核未通过(596代表未开通-请查看config.yml如何开通):%v", result)
				return false, nil
			}
		}
	}
	if result.Code == 596 {
		mylog.Printf("阿里云审核未开通(code=596，请查看config.yml如何开通):%v", result)
	}
