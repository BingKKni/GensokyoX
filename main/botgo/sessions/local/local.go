...
func (l *ChanManager) newConnect(session dto.Session) {
	...
	if err != nil {
		...
		// Resume/Identify 阶段写包即失败（TCP 被 QQ 侧 RST），说明旧 Session 已失效。
		// 根据 QQ 官方文档：错误码 4006 = "无效 session id，请 Identify"；
		// max_concurrency=1 表示每 5s 允许新建 1 个 Session。
		// 因此等待 5s（满足限速要求）后清空 sessionID，下次走新 Identify 而非继续 Resume。
		log.Infof("[ws/session] clearing sessionID and requeueing for fresh Identify in 5s")
