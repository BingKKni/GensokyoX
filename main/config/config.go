...
// 获取是否为纯Webhook模式（跳过QQ Gateway WebSocket连接）
func GetWebhookOnly() bool {
	mu.RLock()
	defer mu.RUnlock()

	if instance == nil {
		fmt.Println("Warning: instance is nil when trying to get webhook_only value.")
		return false
	}
	return instance.Settings.WebhookOnly
}
...
// 获取 WebhookRespCode
// 返回 0 表示兜底走 code 0；返回 1-5 时在无应用端覆盖时用此 code 写进 webhook 200 OK 响应体
func GetWebhookRespCode() int {
	mu.RLock()
	defer mu.RUnlock()

	if instance == nil {
		fmt.Println("Warning: instance is nil when trying to get WebhookRespCode.")
		return 0
	}
	return instance.Settings.WebhookRespCode
}

// 获取 WebhookRespWaitMs，单位毫秒。<=0 表示不等待（立即用兜底 code 响应）。
func GetWebhookRespWaitMs() int {
	mu.RLock()
	defer mu.RUnlock()

	if instance == nil {
		fmt.Println("Warning: instance is nil when trying to get WebhookRespWaitMs.")
		return 0
	}
	return instance.Settings.WebhookRespWaitMs
}
...
// GetDirectRecordURL 获取DirectRecordURL
func GetDirectRecordURL() bool {
	mu.RLock()
	defer mu.RUnlock()

	if instance == nil {
		fmt.Println("Warning: instance is nil when trying to get DirectRecordURL value.")
		return false
	}

	return instance.Settings.DirectRecordURL
}
...
func GetImageLimitB() int {
	mu.RLock()
	defer mu.RUnlock()
	if instance != nil && instance.Settings.ImageLimit > 0 {
		return instance.Settings.ImageLimit
	}
	return 1000
}

// GetUploadPicV2Base64 获取上传图片V2 Base64设置
