		...
		if imageCount == 1 && messageText != "" {
			...
			if !isbase64 {
				...
				newMessage := &dto.MessageToCreate{
					...
					Image:   Reply.Image,
					MsgID:   messageID,
					EventID: "", // 频道消息通常不需要EventID
					MsgSeq:  msgseq,
					MsgType: 0,
				}
				newMessage.Timestamp = time.Now().Unix() // 设置时间戳
				if resp, err = api.PostMessage(context.TODO(), channelID.(string), newMessage); err != nil {
			...
			// 发送成功回执
			retmsg, _ = SendGuildResponse(client, err, &message, resp, api)
		...
		for key, urls := range foundItems {
			for _, url := range urls {
				...
				if isBase64Image {
					...
					if resp, err = api.PostMessage(context.TODO(), channelID.(string), reply); err != nil {
...
func GenerateReplyMessage(id string, foundItems map[string][]string, messageText string, msgseq int, eventid string) (*dto.MessageToCreate, bool) {
