...
// 创建一个带超时的HTTP客户端用于获取图片尺寸
var imageDimensionClient = &http.Client{
	Timeout: 2 * time.Second, // 2秒超时，避免阻塞太久
}
...
func getPNGDimensions(url string) (int, int, error) {
	resp, err := imageDimensionClient.Get(url)
	if err != nil {
...
func getJpegDimensions(url string) (int, int, error) {
	response, err := imageDimensionClient.Get(url)
	if err != nil {
		return 0, 0, fmt.Errorf("error occurred while making request: %w", err)
	}
