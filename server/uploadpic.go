...
func UploadBase64ImageHandler(rateLimiter *RateLimiter) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !rateLimiter.CheckAndUpdateRateLimit(ipAddress) {
			c.JSON(http.StatusTooManyRequests, gin.H{"error": "图片上传频率到达阈值，请稍后再试，或尝试调高 config.yml 中的 image_limit 值"})
			return
		}
...
func UploadBase64ImageHandlerV3(rateLimiter *RateLimiter, apiv1 openapi.OpenAPI) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			...
			c.JSON(http.StatusInternalServerError, gin.H{"error": "image compression failed"})
			return
		}
		...
		if _, err = apiv1.PostMessageMultipart(context.TODO(), channelID, newMessage, compressedData); err != nil {
			...
			if isTimeoutError(err) {
				mylog.Printf("检测到超时错误，但消息可能已发送成功，尝试构造返回结果")
				// 计算压缩数据的MD5值，构造可能的图片URL
		...
		if err != nil {
			...
			// 即使获取宽高失败，我们仍然可以返回 URL，设置默认宽高为 0
			c.JSON(http.StatusOK, gin.H{
				"url":       imageURL,
				"channelID": channelID,
				"width":     0,
...
func UploadBase64RecordHandler(rateLimiter *RateLimiter) gin.HandlerFunc {
	return func(c *gin.Context) {
		ipAddress := c.ClientIP()
		if !rateLimiter.CheckAndUpdateRateLimit(ipAddress) {
			c.JSON(http.StatusTooManyRequests, gin.H{"error": "语音上传频率到达阈值，请稍后再试"})
			return
		}
